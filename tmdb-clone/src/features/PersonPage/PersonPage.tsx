...
import { Table, Icon, Image, Modal } from 'semantic-ui-react';
...
class PersonPage extends Component<RouteComponentProps<PersonPageProps>, PersonPageState> {
  ...
  getAge = (): number | null => {
    if (this.state.model.birthday && this.state.model.deathday) {
      return new Date(this.state.model.deathday).getFullYear() - new Date(this.state.model.birthday).getFullYear();
    } else if (this.state.model.birthday && !this.state.model.deathday) {
          ...
          <div className="personInform-firstColumn">
            ...
            <section className="posterInform-facts">
              <div>
                ...
                <h4>Birthday</h4>
                <div>{this.state.model.birthday} ({this.getAge() ? `${this.getAge()} years old` : ''})</div>
                <h4>Place of Birth</h4>
                <div>{this.state.model.place_of_birth}</div>
                {this.state.model.deathday ? <h4>Deathday</h4> : ''}
                {this.state.model.deathday ? <div>{this.state.model.deathday} ({this.getAge() ? `${this.getAge()} years old` : ''})</div> : ''}
              </div>
              <div>
                <h4>Also Known As</h4>
                {this.state.model.also_known_as?.map((item, index) => (<div key={index}>{item}</div>))}
              </div>
            </section>
          </div>
